...
/**
 * @file ex2.c
 * @brief Ð? bài: Get host's IP address by name
 * Ð?u vào: domain name
 * Ð?u ra: IP address
 *
        ...
        if (presult != NULL)
        {
            ...
            if (count > 0)
            {
                ...
                if (s != INVALID_SOCKET)
                {
                    ...
                    if (error == 0) // N?u connect thành công
                    {
                        char buffer[1024] = {0};
                        // G?i message b?t k? cho trang web
                        // G?i l?nh dúng chu?n giao th?c HTTP (S? tr? v? l?i Moved Permanently nhung ksao)
